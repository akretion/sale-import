...
class TestSaleImportAmazonSeller(TransactionCase, ExtendableMixin):
    ...
    #     chunk_ids = self.env["sale.channel"].amazon_seller_import_orders_chunk_cron()
    #     self.assertTrue(chunk_ids)
    #     for chunk_id in chunk_ids:
    #         self.assertIn("AmazonOrderId", chunk_id.data_str)
    #         self.assertEqual(chunk_id.processor, "sale_channel_importer_amazon_seller")

    def test_import_order_shipped(self):
        with patch(
            "odoo.addons.sale_import_amazon_seller.models.sale_channel.SaleChannel"
            ".amazon_seller_import_orders",
            return_value=data.ORDER_SHIPPED,
        ):
            old_order_ids = self.env["sale.order"].search([])
            self.channel_id.amazon_seller_import_orders_chunk()
